...
class CSVPrinter(metaclass=ABCMeta):
...
class CSVTablePrinter(CSVPrinter):
    def __init__(self, file_name):
        self.__reader = CSVStringReader(file_name)

    def print(self):
        print('<table>')
        lines = self.__reader.readCSV()
        if lines is None:
            return
        for line in lines:
            print('<tr>')
...
class CSVTreePrinter(CSVPrinter):
    def __init__(self, file_name):
        self.__reader = CSVFileReader(file_name)
    def print(self):
        prev_item = []
        while True:
            line = self.__reader.readCSV()
            if line is None:
                return
            just_print = False
